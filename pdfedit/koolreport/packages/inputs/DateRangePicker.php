...
class DateRangePicker extends InputControl
{
    ...
    protected function onInit()
    {
        ...
        $this->getReport()->getResourceManager()->addScriptFileOnBegin(
            $this->getAssetManager()->getAssetUrl('daterangepicker/daterangepicker.js')
        );
        $this->getReport()->getResourceManager()->addCssFile(
            $this->getAssetManager()->getAssetUrl('daterangepicker/daterangepicker.css')
        );
        $this->getReport()->getResourceManager()->addCssFile(
            $this->getAssetManager()->getAssetUrl('daterangepicker/extra.css')
        );
        if($this->value===null)
        {
        ...
        if($this->locale!==null)
        {
            if(gettype($this->locale)=="string")
            {
                //use default locale;
                $this->locale = $this->getLocale($this->locale);
                if($this->format===null)
                {
    ...
    protected function getLocale($name)
    {
        $filepath = dirname(__FILE__)."/daterangepicker_locale/$name.php";
        ...
        {
            throw new \Exception("Could not find the locale [$name]");
        }
    }
