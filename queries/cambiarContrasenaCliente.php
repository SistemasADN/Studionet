<?php
    ...
    if(isset($_SESSION['idUsuario'])){
      ...
      $pass = select_query($con, 
                   'SELECT pollito FROM clientes WHERE idCliente = ?', 'i', [$_SESSION['idUsuario']]);
      if($_POST['contraActual'] == $pass[0]['pollito']){
        if($_POST['contraNueva'] == $_POST['contraNuevaRepetir']){
          if(execute_query($con, 'UPDATE clientes SET pollito = ? WHERE idCliente = ?','si', [$_POST['contraNueva'], $_SESSION['idUsuario']])){
